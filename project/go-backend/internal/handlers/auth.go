...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	verifyURL := h.cfg.CORS.AllowedOrigins[0] + "/verify-email?token=" + verifyToken
...
func (h *AuthHandler) VerifyEmail(c *gin.Context) {
	token := c.Query("token")
	if token == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "validation_error", "message": "Missing verification token"})
		return
	}

	user, err := h.userRepo.GetByVerifyToken(c.Request.Context(), token)
	if err != nil {
	...
	if user.VerifyTokenExpiry != nil && user.VerifyTokenExpiry.Before(time.Now()) {
		c.JSON(http.StatusBadRequest, gin.H{"error": "expired_token", "message": "Verification token has expired. Please request a new one."})
		return
	}
	...
	if err := h.userRepo.Update(c.Request.Context(), user); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal_error", "message": "Failed to verify email"})
		return
	}
...
// ResendVerification generates a new verification token and returns it
