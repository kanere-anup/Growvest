...
func (r *StockRepository) List(ctx context.Context, opts ListOptions) ([]models.Stock, int64, error) {
	...
	allowedOrderBy := map[string]bool{
		...
		"market_cap": true, "exchange": true, "created_at": true,
	}
	orderBy := "symbol"
...
// GetAllStocks returns all stocks (including inactive), used by sync to detect existing records
...
// Upsert creates or updates a stock by symbol
func (r *StockRepository) Upsert(ctx context.Context, stock *models.Stock) error {
	var existing models.Stock
	result := r.db.WithContext(ctx).Where("symbol = ? AND deleted_at IS NULL", stock.Symbol).First(&existing)
