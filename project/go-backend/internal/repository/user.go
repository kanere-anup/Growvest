...
// GetByVerifyToken retrieves a user by verification token
func (r *UserRepository) GetByVerifyToken(ctx context.Context, token string) (*models.User, error) {
	var user models.User
	result := r.db.WithContext(ctx).Where("verify_token = ? AND deleted_at IS NULL", token).First(&user)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, ErrUserNotFound
		}
