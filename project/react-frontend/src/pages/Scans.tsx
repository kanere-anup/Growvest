...
export function Scans() {
  ...
  const { liveProgress } = useScanProgress();
  const [page, setPage] = useState(0);
  const [showNewScanModal, setShowNewScanModal] = useState(false);
  const pageSize = 10;

  // Fetch scans — WebSocket handles live updates; fallback poll every 10s for running scans
  const { data: scansData, isLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      ...
      return hasActiveScans ? 10000 : false;
    },
  });
          ...
          <div className="space-y-4">
            {scans.map((scan, index) => (
              <ScanCard
                ...
                liveProgress={liveProgress[scan.id]}
                onDelete={() => {
...
function ScanCard({
  ...
  const pct = progress && progress.total_stocks > 0
    ? Math.round((progress.processed_stocks / progress.total_stocks) * 100)
    : 0;
      ...
      <div className="p-5">
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Live Progress Bar */}
            {isRunning && progress && progress.total_stocks > 0 && (
              <div className="mt-3">
                <div className="flex items-center justify-between text-xs text-theme-secondary mb-1">
                  <span>{progress.processed_stocks} / {progress.total_stocks} stocks processed</span>
                  <span>{pct}%</span>
                </div>
                ...
                <div className="flex items-center gap-4 mt-1 text-xs text-theme-tertiary">
                  <span className="text-success-500">{progress.successful_stocks} ok</span>
                  {progress.failed_stocks > 0 && (
                    <span className="text-danger-500">{progress.failed_stocks} failed</span>
                  )}
                  {progress.results_count > 0 && (
                    <span className="text-primary-500">{progress.results_count} matches</span>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-center">
              <p className={cn(
                ...
                {progress?.results_count ?? scan.results_count ?? 0}
              </p>
