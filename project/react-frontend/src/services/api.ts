...
export const stocksApi = {
  ...
  // Get unique sectors from stock list
  getSectors: async (): Promise<string[]> => {
    const response = await api.get<PaginatedResponse<Stock>>('/stocks', { params: { limit: 200, active: true } });
    const sectors = [...new Set(response.data.items.map(s => s.sector).filter(Boolean))];
    return sectors.sort();
  },

  // Admin-only operations
  syncStocks: async (): Promise<{ message: string }> => {
    const response = await api.post<{ message: string }>('/admin/stocks/sync');
    return response.data;
  },

  getSyncStatus: async (): Promise<{ is_running: boolean }> => {
    const response = await api.get<{ is_running: boolean }>('/admin/stocks/sync/status');
    return response.data;
  },
...
export const chartApi = {
  getData: async (symbol: string, days = 365): Promise<{
    symbol: string;
    points: Array<{
      date: string;
      open: number;
    ...
    const response = await api.get('/chart/data', { params: { symbol, days } });
    return response.data;
  },
};
...
export const backtestApi = {
  run: async (data: BacktestRequest): Promise<BacktestResult> => {
    const response = await api.post<BacktestResult>('/backtest/run', data);
    return response.data;
  },

  strategies: async (): Promise<StrategyInfo[]> => {
    const response = await api.get<StrategyInfo[]>('/backtest/strategies');
    return response.data;
  },
};
...
export const verifyApi = {
  ...
  resend: async (email: string): Promise<{ message: string }> => {
